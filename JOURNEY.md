...
**Loss function:** Cross-entropy loss over the 16-class patch position predictions, excluding all-black patches (pixels < 0.05) from the accuracy metric to avoid penalising identical empty patches.

**Optimiser:** Training began with SGD (lr=1e-3, batch=4), reaching ~49% validation accuracy. Switching to Adam (lr=1e-3, batch=16) with reduce-on-plateau LR scheduling (halve LR after 10 stagnant epochs, minimum 1e-6) improved convergence to ~59.6% over ~300 epochs across multiple fine-tuning stages with progressively lower learning rates (1e-3 → 5e-4 → 2.5e-4).

**Architecture progression:** The final model adds channel attention (global average pooling → FC → ReLU → FC → sigmoid → channel-wise rescaling) after each max-pooling layer, allowing the network to learn which feature channels are most relevant at each spatial resolution. Pre-trained convolutional weights from the baseline model were transferred into this architecture, so the attention modules trained on top of already-learned features. This reached **68.1%** in 68 epochs with Adam (lr=1e-3).

**Data augmentation:** Random horizontal flip, rotation (±15°, black border fill), brightness/contrast shifts (±30%, foreground-only), Gaussian blur, morphological erosion/dilation (varying stroke thickness), and foreground-only Gaussian noise. A final threshold (pixels < 0.05 → 0) preserves black backgrounds. Additional experiments with inverted dropout (p=0.15) did not improve over the base architecture.
